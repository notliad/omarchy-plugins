...
BarWidget {
  ...
  // The pill is a text label now (just the temperature), so the open-panel
  // dot should track the label like the clock widget does instead of a
  // fraction of whatever slot it fills.
  readonly property real openPanelIndicatorWidth: button.labelWidth
  readonly property real openPanelIndicatorHeight: Math.max(Style.space(10), Math.round(Style.bar.iconSlot * 0.55))

  visible: panelLoader.item && panelLoader.item.barText !== ""
