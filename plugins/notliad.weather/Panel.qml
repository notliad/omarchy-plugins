...
Panel {
  ...
  // What the bar shows: just the current temperature. Empty until the first
  // response lands, so the bar hides exactly like the old icon-only pill did.
  readonly property string barText: root.current ? root.reportTempNum + root.tempUnit : ""
