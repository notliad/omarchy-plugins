...
Panel {
  ...
  onOpenedChanged: {
    if (opened) {
      ...
      outputPortsProc.running = true
      inputPortsProc.running = true
      focusSection = "output"
  ...
  function updateOutputPorts(raw) {
    outputPorts = Model.parsePorts(raw, "output")
  }

  function updateInputPorts(raw) {
    inputPorts = Model.parsePorts(raw, "input")
  }

  function setPort(port) {
    if (!port) return
    var command = port.kind === "output" ? "set-sink-port" : "set-source-port"
    Quickshell.execDetached(["pactl", command, port.device, port.name])
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateOutputPorts(text)
    }
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateInputPorts(text)
    }
  }
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (!inputPortsProc.running) inputPortsProc.running = true
    }
  }
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      ScrollView {
        ...
        Column {
          id: panelColumn
          width: Math.max(0, scrollArea.availableWidth - Style.space(18))
  ...
  component PortRow: CursorSurface {
    ...
    foreground: root.bar.foreground
    fill: root.hoverFill
    current: portRow.port.active
    currentFill: Util.alpha(Color.accent, 0.35)
    implicitHeight: portLabel.implicitHeight + Style.spacing.xl
    ...
    Text {
      ...
      anchors.rightMargin: Style.space(6)
      text: portRow.port.description
      color: root.bar.foreground
      font.family: root.bar.fontFamily
      font.pixelSize: Style.font.body
      font.bold: portRow.port.active
      elide: Text.ElideRight
    }
    ...
    MouseArea {
      ...
      onEntered: portRow.fill = root.hoverFill
      onExited: portRow.fill = "transparent"
      onClicked: root.setPort(portRow.port)
    }
  }
