...
function parsePorts(raw, kind) {
  var devices
  try {
    devices = JSON.parse(String(raw || "[]"))
  } catch (e) {
    return []
  }
  if (!Array.isArray(devices)) devices = devices && Array.isArray(devices.devices) ? devices.devices : []

  var result = []
  for (var i = 0; i < devices.length; i++) {
    var device = devices[i]
    if (!device || !device.name) continue
    if (kind === "input" && (device.monitor_of_sink !== undefined
        || device.monitor_of_sink_name
        || String(device.name).endsWith(".monitor")
        || String((device.properties || {})["device.class"] || "").toLowerCase() === "monitor")) continue
    var ports = Array.isArray(device.ports)
      ? device.ports
      : Object.keys(device.ports || {}).map(function(name) {
          var port = device.ports[name]
          port.name = name
          return port
        })
    for (var j = 0; j < ports.length; j++) {
      ...
      var availability = String(port.availability || port.available || "").toLowerCase()
      if (port.available === false
          || availability === "no" || availability === "not available") continue
      result.push({
        device: String(device.name),
        kind: kind,
        name: String(port.name),
        description: String(port.description || port.name),
        active: String(device.active_port || "") === String(port.name)
      })
    }
  }
  return result
