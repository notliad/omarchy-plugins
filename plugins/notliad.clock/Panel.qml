...
Panel {
  ...
  function weekdayLabel(weekday) {
    return String(Qt.locale("pt_BR").dayName(weekday, Locale.ShortFormat)).replace(/\.$/, "").toUpperCase()
  }

  function monthLabel(month) {
    return String(Qt.locale("pt_BR").monthName(month, Locale.ShortFormat))
      .replace(/\.$/, "")
      .toUpperCase()
  }
  ...
  KeyboardPanel {
    ...
    contentWidth: panel.fittedContentWidth(annualView.width)
    contentHeight: panel.fittedContentHeight(annualView.height)
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        id: calendarScroll
        anchors.fill: parent
        contentWidth: annualView.width
        contentHeight: annualView.height
        clip: true
        ...
        interactive: contentHeight > height || contentWidth > width
          ...
          Item {
            id: annualView
            readonly property real availableWidth: panel.availableCardWidth > 0
              ? panel.availableCardWidth
              : annualGrid.implicitWidth + Style.space(32)
            readonly property real annualScale: Math.min(1, Math.max(0.1,
              (availableWidth - Style.space(32)) / Math.max(1, annualGrid.implicitWidth)))
            width: Math.min(annualGrid.implicitWidth + Style.space(32), availableWidth)
            height: annualGrid.implicitHeight * annualScale

            Column {
              id: annualGrid
              anchors.horizontalCenter: parent.horizontalCenter
              spacing: Style.space(10)
              scale: annualView.annualScale
              ...
              Grid {
                ...
                Repeater {
                  ...
                  Item {
                    ...
                    readonly property int monthIndex: modelData
                    width: Style.space(131)
                    height: Style.space(106)

                    Text {
                      anchors.horizontalCenter: parent.horizontalCenter
                      text: root.monthLabel(modelData)
                    ...
                    Column {
                      ...
                      Repeater {
                        model: Model.monthGrid(root.today.getFullYear(), monthIndex, root.weekStart, root.todayKey)
